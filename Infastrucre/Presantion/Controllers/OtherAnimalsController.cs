...
[ApiController]
[Route("api/other-animals")]
[Authorize]
[Produces("application/json")]
public class OtherAnimalsController : ControllerBase
{
    private readonly IOtherAnimalService _service;

    public OtherAnimalsController(IOtherAnimalService service)
    {
        _service = service;
    }
    ...
    [ProducesResponseType(typeof(ApiResponse<OtherAnimalDetailsDto>), StatusCodes.Status201Created)]
    public async Task<ActionResult<ApiResponse<OtherAnimalDetailsDto>>> Create(
        [FromForm] CreateOtherAnimalRequest request)
    {
        ...
        return StatusCode(StatusCodes.Status201Created,
            ApiResponse<OtherAnimalDetailsDto>.Ok(result, UserMessages.Listings.Created));
    }

    [HttpGet]
    [AllowAnonymous]
    [ProducesResponseType(typeof(ApiResponse<PaginatedResult<OtherAnimalListItemDto>>), StatusCodes.Status200OK)]
    public async Task<ActionResult<ApiResponse<PaginatedResult<OtherAnimalListItemDto>>>> GetList(
        [FromQuery] OtherAnimalFilterParams filter)
    {
        var result = await _service.GetListAsync(filter, HttpContext.RequestAborted);
        return Ok(ApiResponse<PaginatedResult<OtherAnimalListItemDto>>.Ok(
            result, UserMessages.Listings.Loaded));
    }

    [HttpGet("{id:guid}")]
    [AllowAnonymous]
    [ProducesResponseType(typeof(ApiResponse<OtherAnimalDetailsDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ApiResponse<OtherAnimalDetailsDto>>> GetById(Guid id)
    {
        var result = await _service.GetByIdAsync(id, HttpContext.RequestAborted);
        return Ok(ApiResponse<OtherAnimalDetailsDto>.Ok(result, UserMessages.Listings.DetailsLoaded));
    }
    ...
    [ProducesResponseType(typeof(ApiResponse<OtherAnimalDetailsDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ApiResponse<OtherAnimalDetailsDto>>> Update(
        Guid id, [FromForm] UpdateOtherAnimalRequest request)
    {
        ...
        return Ok(ApiResponse<OtherAnimalDetailsDto>.Ok(result, UserMessages.Listings.Updated));
    }
