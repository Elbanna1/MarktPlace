...
[ApiController]
[Route("api/sheep-goats")]
[Authorize]
[Produces("application/json")]
public class SheepGoatsController : ControllerBase
{
    private readonly ISheepGoatService _service;

    public SheepGoatsController(ISheepGoatService service)
    {
        _service = service;
    }
    ...
    [ProducesResponseType(typeof(ApiResponse<SheepGoatDetailsDto>), StatusCodes.Status201Created)]
    public async Task<ActionResult<ApiResponse<SheepGoatDetailsDto>>> Create(
        [FromForm] CreateSheepGoatRequest request)
    {
        ...
        return StatusCode(StatusCodes.Status201Created,
            ApiResponse<SheepGoatDetailsDto>.Ok(result, UserMessages.Listings.Created));
    }

    [HttpGet]
    [AllowAnonymous]
    [ProducesResponseType(typeof(ApiResponse<PaginatedResult<SheepGoatListItemDto>>), StatusCodes.Status200OK)]
    public async Task<ActionResult<ApiResponse<PaginatedResult<SheepGoatListItemDto>>>> GetList(
        [FromQuery] SheepGoatFilterParams filter)
    {
        var result = await _service.GetListAsync(filter, HttpContext.RequestAborted);
        return Ok(ApiResponse<PaginatedResult<SheepGoatListItemDto>>.Ok(
            result, UserMessages.Listings.Loaded));
    }

    [HttpGet("{id:guid}")]
    [AllowAnonymous]
    [ProducesResponseType(typeof(ApiResponse<SheepGoatDetailsDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ApiResponse<SheepGoatDetailsDto>>> GetById(Guid id)
    {
        var result = await _service.GetByIdAsync(id, HttpContext.RequestAborted);
        return Ok(ApiResponse<SheepGoatDetailsDto>.Ok(result, UserMessages.Listings.DetailsLoaded));
    }
    ...
    [ProducesResponseType(typeof(ApiResponse<SheepGoatDetailsDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ApiResponse<SheepGoatDetailsDto>>> Update(
        Guid id, [FromForm] UpdateSheepGoatRequest request)
    {
        ...
        return Ok(ApiResponse<SheepGoatDetailsDto>.Ok(result, UserMessages.Listings.Updated));
    }
