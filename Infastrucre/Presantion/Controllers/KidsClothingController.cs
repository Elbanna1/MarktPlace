...
[ApiController]
[Route("api/kids-clothing")]
[Authorize]
[Produces("application/json")]
public class KidsClothingController : ControllerBase
{
    private readonly IKidsClothingService _service;

    public KidsClothingController(IKidsClothingService service)
    {
        _service = service;
    }
    ...
    [ProducesResponseType(typeof(ApiResponse<KidsClothingDetailsDto>), StatusCodes.Status201Created)]
    public async Task<ActionResult<ApiResponse<KidsClothingDetailsDto>>> Create(
        [FromForm] CreateKidsClothingRequest request)
    {
        ...
        return StatusCode(StatusCodes.Status201Created,
            ApiResponse<KidsClothingDetailsDto>.Ok(result, UserMessages.Listings.Created));
    }

    [HttpGet]
    [AllowAnonymous]
    [ProducesResponseType(typeof(ApiResponse<PaginatedResult<KidsClothingListItemDto>>), StatusCodes.Status200OK)]
    public async Task<ActionResult<ApiResponse<PaginatedResult<KidsClothingListItemDto>>>> GetList(
        [FromQuery] KidsClothingFilterParams filter)
    {
        var result = await _service.GetListAsync(filter, HttpContext.RequestAborted);
        return Ok(ApiResponse<PaginatedResult<KidsClothingListItemDto>>.Ok(
            result, UserMessages.Listings.Loaded));
    }

    [HttpGet("{id:guid}")]
    [AllowAnonymous]
    [ProducesResponseType(typeof(ApiResponse<KidsClothingDetailsDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ApiResponse<KidsClothingDetailsDto>>> GetById(Guid id)
    {
        var result = await _service.GetByIdAsync(id, HttpContext.RequestAborted);
        return Ok(ApiResponse<KidsClothingDetailsDto>.Ok(result, UserMessages.Listings.DetailsLoaded));
    }
    ...
    [ProducesResponseType(typeof(ApiResponse<KidsClothingDetailsDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ApiResponse<KidsClothingDetailsDto>>> Update(
        Guid id, [FromForm] UpdateKidsClothingRequest request)
    {
        var cancellationToken = HttpContext.RequestAborted;
        ...
        return Ok(ApiResponse<KidsClothingDetailsDto>.Ok(result, UserMessages.Listings.Updated));
    }
