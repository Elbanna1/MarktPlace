...
public class BirdService : IBirdService
{
    ...
    public async Task<BirdDetailsDto> CreateAsync(
        string userId,
        CreateBirdRequest request,
        IReadOnlyList<UploadImageModel> images)
    {
        var entity = _mapper.Map<Bird>(request);
        ...
        await _notifications.NotifyAsync(
            userId, ListingModuleType.Bird, NotificationAction.Created, entity.Id, entity.Title);

        return await BuildDetailsAsync(entity.Id, includeUnmoderated: true);
    }

    public async Task<BirdDetailsDto> UpdateAsync(
        string userId,
        bool isAdmin,
        Guid id,
        ...
        IReadOnlyList<UploadImageModel> newImages)
    {
        ...
        if (entity is null)
            throw new NotFoundException("إعلان الطيور مش موجود.");
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.Bird,
    ...
    public async Task<PaginatedResult<BirdListItemDto>> GetListAsync(
        BirdFilterParams filter, CancellationToken cancellationToken = default)
    {
        ...
        var mapped = _mapper.Map<IReadOnlyList<BirdListItemDto>>(items);
        return new PaginatedResult<BirdListItemDto>(
            mapped, total, filter.PageIndex, filter.PageSize);
    }

    public Task<BirdDetailsDto> GetByIdAsync(
    ...
    private async Task<BirdDetailsDto> BuildDetailsAsync(
        Guid id, CancellationToken cancellationToken = default, bool includeUnmoderated = false)
    {
        ...
        return _mapper.Map<BirdDetailsDto>(entity);
    }

    private Task<List<BirdImage>> AttachImagesAsync(
        Bird entity, IReadOnlyList<UploadImageModel> images, bool startAsPrimary) =>
        ListingImages.AttachAsync(
            _fileService,
            entity.Images,
            images,
            ImageConstants.BirdFolder,
            startAsPrimary,
            _ => new BirdImage { BirdId = entity.Id });
}
