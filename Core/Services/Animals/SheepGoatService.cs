...
public class SheepGoatService : ISheepGoatService
{
    private readonly ISheepGoatRepository _repository;
    private readonly IFileService _fileService;
    private readonly IMapper _mapper;
    private readonly INotificationService _notifications;

    public SheepGoatService(
        ISheepGoatRepository repository,
    ...
    public async Task<SheepGoatDetailsDto> CreateAsync(
        string userId,
        CreateSheepGoatRequest request,
        IReadOnlyList<UploadImageModel> images)
    {
        var entity = _mapper.Map<SheepGoat>(request);
        ...
        await _notifications.NotifyAsync(
            userId, ListingModuleType.SheepGoat, NotificationAction.Created, entity.Id, entity.Title);

        return await BuildDetailsAsync(entity.Id, includeUnmoderated: true);
    }

    public async Task<SheepGoatDetailsDto> UpdateAsync(
        string userId,
        bool isAdmin,
        Guid id,
        ...
        IReadOnlyList<UploadImageModel> newImages)
    {
        ...
        if (entity is null)
            throw new NotFoundException("إعلان الأغنام والماعز مش موجود.");
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.SheepGoat,
    ...
    public async Task<PaginatedResult<SheepGoatListItemDto>> GetListAsync(
        SheepGoatFilterParams filter, CancellationToken cancellationToken = default)
    {
        ...
        var mapped = _mapper.Map<IReadOnlyList<SheepGoatListItemDto>>(items);
        return new PaginatedResult<SheepGoatListItemDto>(
            mapped, total, filter.PageIndex, filter.PageSize);
    }

    public Task<SheepGoatDetailsDto> GetByIdAsync(
    ...
    private async Task<SheepGoatDetailsDto> BuildDetailsAsync(
        Guid id, CancellationToken cancellationToken = default, bool includeUnmoderated = false)
    {
        ...
        return _mapper.Map<SheepGoatDetailsDto>(entity);
    }

    private Task<List<SheepGoatImage>> AttachImagesAsync(
        SheepGoat entity, IReadOnlyList<UploadImageModel> images, bool startAsPrimary) =>
        ListingImages.AttachAsync(
            _fileService,
            entity.Images,
            images,
            ImageConstants.SheepGoatFolder,
            startAsPrimary,
            _ => new SheepGoatImage { SheepGoatId = entity.Id });
}
