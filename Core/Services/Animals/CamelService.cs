...
public class CamelService : ICamelService
{
    ...
    public async Task<CamelDetailsDto> CreateAsync(
        string userId,
        CreateCamelRequest request,
        IReadOnlyList<UploadImageModel> images)
    {
        var entity = _mapper.Map<Camel>(request);
        ...
        await _notifications.NotifyAsync(
            userId, ListingModuleType.Camel, NotificationAction.Created, entity.Id, entity.Title);

        return await BuildDetailsAsync(entity.Id, includeUnmoderated: true);
    }

    public async Task<CamelDetailsDto> UpdateAsync(
        string userId,
        bool isAdmin,
        Guid id,
        ...
        IReadOnlyList<UploadImageModel> newImages)
    {
        ...
        if (entity is null)
            throw new NotFoundException("إعلان الإبل مش موجود.");
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.Camel,
    ...
    public async Task<PaginatedResult<CamelListItemDto>> GetListAsync(
        CamelFilterParams filter, CancellationToken cancellationToken = default)
    {
        ...
        var mapped = _mapper.Map<IReadOnlyList<CamelListItemDto>>(items);
        return new PaginatedResult<CamelListItemDto>(
            mapped, total, filter.PageIndex, filter.PageSize);
    }

    public Task<CamelDetailsDto> GetByIdAsync(
    ...
    private async Task<CamelDetailsDto> BuildDetailsAsync(
        Guid id, CancellationToken cancellationToken = default, bool includeUnmoderated = false)
    {
        ...
        return _mapper.Map<CamelDetailsDto>(entity);
    }

    private Task<List<CamelImage>> AttachImagesAsync(
        Camel entity, IReadOnlyList<UploadImageModel> images, bool startAsPrimary) =>
        ListingImages.AttachAsync(
            _fileService,
            entity.Images,
            images,
            ImageConstants.CamelFolder,
            startAsPrimary,
            _ => new CamelImage { CamelId = entity.Id });
}
