...
public class HorseService : IHorseService
{
    ...
    public async Task<HorseDetailsDto> CreateAsync(
        string userId,
        CreateHorseRequest request,
        IReadOnlyList<UploadImageModel> images)
    {
        var entity = _mapper.Map<Horse>(request);
        ...
        await _notifications.NotifyAsync(
            userId, ListingModuleType.Horse, NotificationAction.Created, entity.Id, entity.Title);

        return await BuildDetailsAsync(entity.Id, includeUnmoderated: true);
    }

    public async Task<HorseDetailsDto> UpdateAsync(
        string userId,
        bool isAdmin,
        Guid id,
        ...
        IReadOnlyList<UploadImageModel> newImages)
    {
        ...
        if (entity is null)
            throw new NotFoundException("إعلان الخيول مش موجود.");
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.Horse,
    ...
    public async Task<PaginatedResult<HorseListItemDto>> GetListAsync(
        HorseFilterParams filter, CancellationToken cancellationToken = default)
    {
        ...
        var mapped = _mapper.Map<IReadOnlyList<HorseListItemDto>>(items);
        return new PaginatedResult<HorseListItemDto>(
            mapped, total, filter.PageIndex, filter.PageSize);
    }

    public Task<HorseDetailsDto> GetByIdAsync(
    ...
    private async Task<HorseDetailsDto> BuildDetailsAsync(
        Guid id, CancellationToken cancellationToken = default, bool includeUnmoderated = false)
    {
        ...
        return _mapper.Map<HorseDetailsDto>(entity);
    }

    private Task<List<HorseImage>> AttachImagesAsync(
        Horse entity, IReadOnlyList<UploadImageModel> images, bool startAsPrimary) =>
        ListingImages.AttachAsync(
            _fileService,
            entity.Images,
            images,
            ImageConstants.HorseFolder,
            startAsPrimary,
            _ => new HorseImage { HorseId = entity.Id });
}
