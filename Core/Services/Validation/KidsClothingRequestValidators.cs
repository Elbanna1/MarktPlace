...
public class CreateKidsClothingRequestValidator : AbstractValidator<CreateKidsClothingRequest>
{
    public CreateKidsClothingRequestValidator()
    {
        ...
        RuleFor(x => x.OtherClothingType)
            ...
            .MaximumLength(150)
            .When(x => x.ClothingType == KidsClothingType.Other);
        ...
        RuleFor(x => x.OtherBrand)
            ...
            .MaximumLength(150)
            .When(x => x.Brand == KidsClothingBrand.Other);

        RuleFor(x => x.Sizes)
            .NotEmpty().WithMessage("لازم تختار مقاس واحد على الأقل.")
            .Must(sizes => sizes.All(size => Enum.IsDefined(typeof(KidsClothingSize), size)))
            .WithMessage("فيه مقاس أو أكتر من اللي اخترتهم مش صحيح.");

        RuleFor(x => x.Colors)
            .NotEmpty().WithMessage("لازم تختار لون واحد على الأقل.")
            .Must(colors => colors.All(color => Enum.IsDefined(typeof(KidsClothingColor), color)))
        ...
        RuleFor(x => x.OtherColor)
            ...
            .MaximumLength(150)
            .When(x => x.Colors.Contains(KidsClothingColor.Other));
...
public class UpdateKidsClothingRequestValidator : AbstractValidator<UpdateKidsClothingRequest>
{
    public UpdateKidsClothingRequestValidator()
    {
        ClothingValidationRules.StoreName(this, x => x.StoreName);
