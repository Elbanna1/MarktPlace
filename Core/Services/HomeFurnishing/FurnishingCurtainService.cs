...
public class FurnishingCurtainService : IFurnishingCurtainService
{
    private readonly IFurnishingCurtainRepository _repository;
    private readonly IFileService _fileService;
    private readonly IMapper _mapper;
    private readonly INotificationService _notifications;

    public FurnishingCurtainService(
        IFurnishingCurtainRepository repository,
    ...
    public async Task<FurnishingCurtainDetailsDto> CreateAsync(
        string userId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var entity = new FurnishingCurtain
        {
            Id = Guid.NewGuid(),
            UserId = userId,
        ...
        try
        {
            ...
            if (video is not null)
            {
                var storedVideo = await _fileService.SaveVideoAsync(
                    video, FileUploadConstants.FurnishingCurtainVideoFolder, cancellationToken);
        ...
        await _notifications.NotifyAsync(
            userId, ListingModuleType.FurnishingCurtain, NotificationAction.Created, entity.Id, entity.Title);

        return await BuildDetailsAsync(entity.Id, cancellationToken, includeUnmoderated: true);
    }

    public async Task<FurnishingCurtainDetailsDto> UpdateAsync(
        string userId,
        bool isAdmin,
        Guid id,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.FurnishingCurtain,
    ...
    public async Task DeleteAsync(string userId, Guid id, bool isAdmin)
    {
        ...
        if (entity is null)
            throw new NotFoundException("إعلان المفروشات والستائر مش موجود.");
    ...
    public async Task<PaginatedResult<FurnishingCurtainListItemDto>> GetListAsync(
        FurnishingCurtainFilterParams filter, CancellationToken cancellationToken = default)
    {
        ...
        var mapped = _mapper.Map<IReadOnlyList<FurnishingCurtainListItemDto>>(items);
        return new PaginatedResult<FurnishingCurtainListItemDto>(
            mapped, total, filter.PageIndex, filter.PageSize);
    }

    public async Task<FurnishingCurtainDetailsDto> GetByIdAsync(
    ...
    public async Task<IReadOnlyList<FurnishingCurtainListItemDto>> GetSimilarAsync(
        Guid id, int count, CancellationToken cancellationToken = default) =>
        _mapper.Map<IReadOnlyList<FurnishingCurtainListItemDto>>(
            await _repository.GetSimilarAsync(id, HomeFurnishingListings.StripSize(count), cancellationToken));

    public async Task<IReadOnlyList<FurnishingCurtainListItemDto>> GetRelatedAsync(
        Guid id, int count, CancellationToken cancellationToken = default) =>
        _mapper.Map<IReadOnlyList<FurnishingCurtainListItemDto>>(
            await _repository.GetRelatedAsync(id, HomeFurnishingListings.StripSize(count), cancellationToken));

    public async Task<IReadOnlyList<FurnishingCurtainListItemDto>> GetRecentlyAddedAsync(
    ...
    public async Task<FurnishingCurtainDetailsDto> ReorderImagesAsync(
    ...
    public async Task<FurnishingCurtainDetailsDto> SetPromotionAsync(
        Guid id, PromoteHomeFurnishingRequest request, CancellationToken cancellationToken = default)
    {
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.FurnishingCurtain, NotificationAction.AdminUpdated, id, entity.Title);
    ...
    public async Task<IReadOnlyList<HomeFurnishingLookupItemDto>> GetSizesAsync(
        CancellationToken cancellationToken = default) =>
        _mapper.Map<IReadOnlyList<HomeFurnishingLookupItemDto>>(
    ...
    private async Task<FurnishingCurtain> LoadForWriteAsync(
        string userId, bool isAdmin, Guid id, CancellationToken cancellationToken)
    {
        ...
        return entity ?? throw new NotFoundException("إعلان المفروشات والستائر مش موجود.");
    }

    private static void ApplyEditableFields(FurnishingCurtain entity, CreateFurnishingCurtainRequest request)
    {
        ...
        entity.OtherProductType = HomeFurnishingListings.OtherWhen(
            request.ProductType == FurnishingCurtainProductType.Other, request.OtherProductType);
        entity.Size = request.Size;
        entity.OtherSize = HomeFurnishingListings.OtherWhen(
            request.Size == FurnishingCurtainSize.Other, request.OtherSize);
        entity.Material = request.Material;
        entity.OtherMaterial = HomeFurnishingListings.OtherWhen(
            request.Material == FurnishingCurtainMaterial.Other, request.OtherMaterial);
        entity.OtherColor = HomeFurnishingListings.OtherWhen(
            request.Colors.Contains(FurnishingCurtainColor.Other), request.OtherColor);
    ...
    private static List<FurnishingCurtainColorSelection> BuildColors(
        Guid listingId, IEnumerable<FurnishingCurtainColor> colors) =>
        colors.Distinct()
            .Select(color => new FurnishingCurtainColorSelection { FurnishingCurtainId = listingId, Color = color })
            .ToList();

    private async Task ReplaceColorsAsync(FurnishingCurtain entity, IEnumerable<FurnishingCurtainColor> requested)
    {
    ...
    private async Task<FurnishingCurtainDetailsDto> BuildDetailsAsync(
        Guid id, CancellationToken cancellationToken = default, bool includeUnmoderated = false)
    {
        ...
        var details = _mapper.Map<FurnishingCurtainDetailsDto>(entity);

        details.ShareUrl = HomeFurnishingListings.ShareUrl(
            _fileService, HomeFurnishingRoutes.FurnishingCurtains, id);

        return details;
    }

    private Task<List<FurnishingCurtainImage>> AttachImagesAsync(
        FurnishingCurtain entity,
        IReadOnlyList<UploadImageModel> images,
        ...
        ListingImages.AttachOrderedAsync(
            ...
            entity.Images,
            images,
            ImageConstants.FurnishingCurtainFolder,
            _ => new FurnishingCurtainImage { FurnishingCurtainId = entity.Id },
            cancellationToken);
}
