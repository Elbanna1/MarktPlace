...
{
    private readonly IKitchenToolRepository _repository;
    private readonly IFileService _fileService;
    private readonly IMapper _mapper;
    private readonly INotificationService _notifications;

    public KitchenToolService(
        IKitchenToolRepository repository,
    ...
    public async Task<KitchenToolDetailsDto> CreateAsync(
        string userId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (video is not null)
            {
                var storedVideo = await _fileService.SaveVideoAsync(
                    video, FileUploadConstants.KitchenToolVideoFolder, cancellationToken);
        ...
        await _notifications.NotifyAsync(
            userId, ListingModuleType.KitchenTool, NotificationAction.Created, entity.Id, entity.Title);

        return await BuildDetailsAsync(entity.Id, cancellationToken, includeUnmoderated: true);
    }

    public async Task<KitchenToolDetailsDto> UpdateAsync(
        string userId,
        bool isAdmin,
        Guid id,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.KitchenTool,
    ...
    public async Task DeleteAsync(string userId, Guid id, bool isAdmin)
    {
        ...
        if (entity is null)
            throw new NotFoundException("إعلان أدوات المطبخ مش موجود.");
    ...
    public async Task<PaginatedResult<KitchenToolListItemDto>> GetListAsync(
        KitchenToolFilterParams filter, CancellationToken cancellationToken = default)
    {
        ...
        var mapped = _mapper.Map<IReadOnlyList<KitchenToolListItemDto>>(items);
        return new PaginatedResult<KitchenToolListItemDto>(
            mapped, total, filter.PageIndex, filter.PageSize);
    }

    public async Task<KitchenToolDetailsDto> GetByIdAsync(
    ...
    public async Task<IReadOnlyList<KitchenToolListItemDto>> GetSimilarAsync(
        Guid id, int count, CancellationToken cancellationToken = default) =>
        _mapper.Map<IReadOnlyList<KitchenToolListItemDto>>(
            await _repository.GetSimilarAsync(id, HomeFurnishingListings.StripSize(count), cancellationToken));

    public async Task<IReadOnlyList<KitchenToolListItemDto>> GetRelatedAsync(
        Guid id, int count, CancellationToken cancellationToken = default) =>
        _mapper.Map<IReadOnlyList<KitchenToolListItemDto>>(
            await _repository.GetRelatedAsync(id, HomeFurnishingListings.StripSize(count), cancellationToken));

    public async Task<IReadOnlyList<KitchenToolListItemDto>> GetRecentlyAddedAsync(
    ...
    public async Task<KitchenToolDetailsDto> ReorderImagesAsync(
    ...
    public async Task<KitchenToolDetailsDto> SetPromotionAsync(
        Guid id, PromoteHomeFurnishingRequest request, CancellationToken cancellationToken = default)
    {
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.KitchenTool, NotificationAction.AdminUpdated, id, entity.Title);
    ...
    private async Task<KitchenTool> LoadForWriteAsync(
        string userId, bool isAdmin, Guid id, CancellationToken cancellationToken)
    {
        ...
        return entity ?? throw new NotFoundException("إعلان أدوات المطبخ مش موجود.");
    }

    private static void ApplyEditableFields(KitchenTool entity, CreateKitchenToolRequest request)
    {
        ...
        entity.OtherProductType = HomeFurnishingListings.OtherWhen(
            request.ProductType == KitchenToolProductType.Other, request.OtherProductType);
        entity.Material = request.Material;
        entity.OtherMaterial = HomeFurnishingListings.OtherWhen(
            request.Material == KitchenToolMaterial.Other, request.OtherMaterial);
        entity.OtherColor = HomeFurnishingListings.OtherWhen(
            request.Colors.Contains(KitchenToolColor.Other), request.OtherColor);
    ...
    private static List<KitchenToolColorSelection> BuildColors(
        Guid listingId, IEnumerable<KitchenToolColor> colors) =>
        colors.Distinct()
            .Select(color => new KitchenToolColorSelection { KitchenToolId = listingId, Color = color })
            .ToList();

    private async Task ReplaceColorsAsync(KitchenTool entity, IEnumerable<KitchenToolColor> requested)
    {
    ...
    private async Task<KitchenToolDetailsDto> BuildDetailsAsync(
        Guid id, CancellationToken cancellationToken = default, bool includeUnmoderated = false)
    {
        ...
        var details = _mapper.Map<KitchenToolDetailsDto>(entity);

        details.ShareUrl = HomeFurnishingListings.ShareUrl(
            _fileService, HomeFurnishingRoutes.KitchenTools, id);

        return details;
    }

    private Task<List<KitchenToolImage>> AttachImagesAsync(
        KitchenTool entity,
        IReadOnlyList<UploadImageModel> images,
        ...
        ListingImages.AttachOrderedAsync(
            ...
            entity.Images,
            images,
            ImageConstants.KitchenToolFolder,
            _ => new KitchenToolImage { KitchenToolId = entity.Id },
            cancellationToken);
}
