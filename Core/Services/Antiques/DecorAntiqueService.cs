...
public class DecorAntiqueService : IDecorAntiqueService
{
    private readonly IDecorAntiqueRepository _repository;
    private readonly IFileService _fileService;
    private readonly IMapper _mapper;
    private readonly INotificationService _notifications;

    public DecorAntiqueService(
        IDecorAntiqueRepository repository,
    ...
    public async Task<DecorAntiqueDetailsDto> CreateAsync(
        string userId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var entity = _mapper.Map<DecorAntique>(request);

        entity.Id = Guid.NewGuid();
        entity.UserId = userId;
        entity.Governorate = LocationConstants.Governorate;
        entity.CreatedAt = DateTime.UtcNow;
        entity.OtherItemType = request.OtherItemType?.Trim();
        ...
        try
        {
            ...
            if (video is not null)
            {
                entity.Video = await ListingVideos.SaveAsync(
                    _fileService, video, FileUploadConstants.DecorAntiqueVideoFolder,
                    _ => new DecorAntiqueVideo { DecorAntiqueId = entity.Id }, cancellationToken);
        ...
        await _notifications.NotifyAsync(
            userId, ListingModuleType.DecorAntique, NotificationAction.Created, entity.Id, entity.Title);

        return await BuildDetailsAsync(entity.Id, cancellationToken, includeUnmoderated: true);
    }

    public async Task<DecorAntiqueDetailsDto> UpdateAsync(
        string userId,
        bool isAdmin,
        Guid id,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (entity is null)
            throw new NotFoundException("إعلان ديكور التحف مش موجود.");
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.DecorAntique,
    ...
    public async Task<PaginatedResult<DecorAntiqueListItemDto>> GetListAsync(
        DecorAntiqueFilterParams filter, CancellationToken cancellationToken = default)
    {
        ...
        var mapped = _mapper.Map<IReadOnlyList<DecorAntiqueListItemDto>>(items);
        return new PaginatedResult<DecorAntiqueListItemDto>(
            mapped, total, filter.PageIndex, filter.PageSize);
    }

    public Task<DecorAntiqueDetailsDto> GetByIdAsync(
    ...
    private async Task<DecorAntiqueDetailsDto> BuildDetailsAsync(
        Guid id, CancellationToken cancellationToken = default, bool includeUnmoderated = false)
    {
        ...
        return _mapper.Map<DecorAntiqueDetailsDto>(entity);
    }

    private Task<List<DecorAntiqueImage>> AttachImagesAsync(
        DecorAntique entity, IReadOnlyList<UploadImageModel> images, bool startAsPrimary) =>
        ListingImages.AttachAsync(
            _fileService,
            entity.Images,
            images,
            ImageConstants.DecorAntiqueFolder,
            startAsPrimary,
            _ => new DecorAntiqueImage { DecorAntiqueId = entity.Id });
}
