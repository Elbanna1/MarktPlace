...
public class AntiqueService : IAntiqueService
{
    private readonly IAntiqueRepository _repository;
    private readonly IFileService _fileService;
    private readonly IMapper _mapper;
    private readonly INotificationService _notifications;

    public AntiqueService(
        IAntiqueRepository repository,
    ...
    public async Task<AntiqueDetailsDto> CreateAsync(
        string userId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var entity = _mapper.Map<Antique>(request);
        ...
        entity.CountryOfOrigin = request.CountryOfOrigin?.Trim();
        ...
        try
        {
            ...
            if (video is not null)
            {
                entity.Video = await ListingVideos.SaveAsync(
                    _fileService, video, FileUploadConstants.AntiqueVideoFolder,
                    _ => new AntiqueVideo { AntiqueId = entity.Id }, cancellationToken);
        ...
        await _notifications.NotifyAsync(
            userId, ListingModuleType.Antique, NotificationAction.Created, entity.Id, entity.Title);

        return await BuildDetailsAsync(entity.Id, cancellationToken, includeUnmoderated: true);
    }

    public async Task<AntiqueDetailsDto> UpdateAsync(
        string userId,
        bool isAdmin,
        Guid id,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (entity is null)
            throw new NotFoundException("إعلان التحفة مش موجود.");

        entity.SellerName = request.SellerName;
        entity.Phone = request.Phone;
        entity.WhatsApp = request.WhatsApp?.Trim();
        entity.AntiqueName = request.AntiqueName;
        entity.AntiqueType = request.AntiqueType;
        entity.OtherType = request.OtherType?.Trim();
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.Antique,
    ...
    public async Task<PaginatedResult<AntiqueListItemDto>> GetListAsync(
        AntiqueFilterParams filter, CancellationToken cancellationToken = default)
    {
        ...
        var mapped = _mapper.Map<IReadOnlyList<AntiqueListItemDto>>(items);
        return new PaginatedResult<AntiqueListItemDto>(
            mapped, total, filter.PageIndex, filter.PageSize);
    }

    public Task<AntiqueDetailsDto> GetByIdAsync(
    ...
    public async Task<IReadOnlyList<AntiqueLookupItemDto>> GetWorkingStatusesAsync(
        CancellationToken cancellationToken = default) =>
        _mapper.Map<IReadOnlyList<AntiqueLookupItemDto>>(
            await _repository.GetWorkingStatusesAsync(cancellationToken));
    ...
    private async Task<AntiqueDetailsDto> BuildDetailsAsync(
        Guid id, CancellationToken cancellationToken = default, bool includeUnmoderated = false)
    {
        ...
        return _mapper.Map<AntiqueDetailsDto>(entity);
    }

    private Task<List<AntiqueImage>> AttachImagesAsync(
        Antique entity, IReadOnlyList<UploadImageModel> images, bool startAsPrimary) =>
        ListingImages.AttachAsync(
            _fileService,
            entity.Images,
            images,
            ImageConstants.AntiqueFolder,
            startAsPrimary,
            _ => new AntiqueImage { AntiqueId = entity.Id });
}
