...
public class WomenClothingService : IWomenClothingService
{
    private readonly IWomenClothingRepository _repository;
    private readonly IFileService _fileService;
    private readonly IMapper _mapper;
    private readonly INotificationService _notifications;

    public WomenClothingService(
        IWomenClothingRepository repository,
    ...
    public async Task<WomenClothingDetailsDto> CreateAsync(
        string userId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var entity = _mapper.Map<WomenClothing>(request);
        ...
        try
        {
            ...
            if (video is not null)
            {
                var storedVideo = await _fileService.SaveVideoAsync(
                    video, FileUploadConstants.WomenClothingVideoFolder, cancellationToken);
        ...
        await _notifications.NotifyAsync(
            userId, ListingModuleType.WomenClothing, NotificationAction.Created, entity.Id, entity.Title);

        return await BuildDetailsAsync(entity.Id, cancellationToken, includeUnmoderated: true);
    }

    public async Task<WomenClothingDetailsDto> UpdateAsync(
        string userId,
        bool isAdmin,
        Guid id,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (entity is null)
            throw new NotFoundException("إعلان الملابس الحريمي مش موجود.");
        ...
        await _notifications.NotifyAsync(
            entity.UserId, ListingModuleType.WomenClothing,
    ...
    public async Task<PaginatedResult<WomenClothingListItemDto>> GetListAsync(
        WomenClothingFilterParams filter, CancellationToken cancellationToken = default)
    {
        ...
        var mapped = _mapper.Map<IReadOnlyList<WomenClothingListItemDto>>(items);
        return new PaginatedResult<WomenClothingListItemDto>(
            mapped, total, filter.PageIndex, filter.PageSize);
    }

    public Task<WomenClothingDetailsDto> GetByIdAsync(
    ...
    private static string? NormalizeGoogleMaps(WomenClothingSellingMethod sellingMethod, string? googleMaps) =>
        sellingMethod is WomenClothingSellingMethod.Store or WomenClothingSellingMethod.StoreAndOnline
            ? string.IsNullOrWhiteSpace(googleMaps) ? null : googleMaps.Trim()
            : null;

    private static string? NormalizeOtherColor(
        IEnumerable<WomenClothingColor> colors, string? otherColor) =>
        colors.Contains(WomenClothingColor.Other)
            ? string.IsNullOrWhiteSpace(otherColor) ? null : otherColor.Trim()
            : null;

    private static List<WomenClothingSizeSelection> BuildSizes(Guid listingId, IEnumerable<WomenClothingSize> sizes) =>
        sizes.Distinct()
            .Select(size => new WomenClothingSizeSelection { WomenClothingId = listingId, Size = size })
            .ToList();

    private static List<WomenClothingColorSelection> BuildColors(Guid listingId, IEnumerable<WomenClothingColor> colors) =>
        colors.Distinct()
            .Select(color => new WomenClothingColorSelection { WomenClothingId = listingId, Color = color })
            .ToList();

    private async Task ReplaceSizesAsync(WomenClothing entity, IEnumerable<WomenClothingSize> requested)
    {
    ...
    private async Task ReplaceColorsAsync(WomenClothing entity, IEnumerable<WomenClothingColor> requested)
    {
        var wanted = requested.Distinct().ToHashSet();
    ...
    private async Task<WomenClothingDetailsDto> BuildDetailsAsync(
        Guid id, CancellationToken cancellationToken = default, bool includeUnmoderated = false)
    {
        ...
        return _mapper.Map<WomenClothingDetailsDto>(entity);
    }

    private Task<List<WomenClothingImage>> AttachImagesAsync(
        WomenClothing entity,
        IReadOnlyList<UploadImageModel> images,
        bool startAsPrimary,
        ...
        ListingImages.AttachAsync(
            ...
            entity.Images,
            images,
            ImageConstants.WomenClothingFolder,
            startAsPrimary,
            _ => new WomenClothingImage { WomenClothingId = entity.Id },
            cancellationToken);
}
